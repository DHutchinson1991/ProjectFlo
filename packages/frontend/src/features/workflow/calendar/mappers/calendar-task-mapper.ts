...
export function transformBackendTasks(backendTasks: BackendCalendarTask[], timezone = 'UTC'): CalendarTask[] {
    return backendTasks
        .filter(t => t.due_date)
        .map(t => {
            ...
            // Compare date strings in brand timezone for accurate overdue logic
            const dueDateStr = t.due_date!.split('T')[0];
            const todayStr = new Date().toLocaleDateString('en-CA', { timeZone: timezone });
            const isOverdue = !isCompleted && dueDateStr < todayStr;
            ...
            return {
                ...
                assignee: t.assignee
                    ? { id: t.assignee.id.toString(), name: t.assignee.name, email: t.assignee.email, role: 'Crew' }
                    : undefined,
