...
import type { SectionBaseProps, PublicProposalQuote } from "@/features/workflow/proposals/types";
import RevealBox from "./RevealBox";

interface QuoteSectionProps extends SectionBaseProps {
    quote: PublicProposalQuote;
}

/* ── Category colour palette (matches finance/shared) ── */
...
export default function QuoteSection({ quote, colors, cardSx }: QuoteSectionProps) {
    ...
    /* ── Totals ── */
    const subtotal = quote.items.reduce((s, i) => s + computeLineTotal(i.quantity, i.unit_price), 0);
    ...
    const deposit = quote.deposit_required ? parseFloat(String(quote.deposit_required)) : null;
    ...
    return (
        <RevealBox>
            <Box sx={{ ...cardSx, p: 0, overflow: "hidden" }}>
                ...
                <Box sx={{
                    ...
                    <Chip
                        ...
                        sx={{
                            ...
                            bgcolor: alpha(quote.status === "Accepted" ? "#22c55e" : colors.accent, 0.12),
                            color: quote.status === "Accepted" ? "#22c55e" : colors.accent,
                            border: `1px solid ${alpha(quote.status === "Accepted" ? "#22c55e" : colors.accent, 0.25)}`,
                        }}
                    />
                </Box>
                    ...
                    <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1, rowGap: 0.5 }}>
                        ...
                        {taxAmount > 0 && (
                            <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                                ...
                                <Typography sx={{ fontSize: "0.64rem", fontWeight: 600, color: colors.muted }}>
                                    Tax ({taxRate}%)
                                </Typography>
                                <Typography sx={{ fontSize: "0.64rem", fontWeight: 700, color: alpha(colors.text, 0.65), fontFamily: "monospace" }}>
                ...
                {/* ── Footer totals ── */}
                <Box sx={{ px: { xs: 2.5, md: 3 }, py: 2, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    {deposit !== null && (
                        <Typography sx={{ fontSize: "0.72rem", color: colors.muted }}>
                            Deposit: <span style={{ fontFamily: "monospace", fontWeight: 700, color: alpha(colors.text, 0.7) }}>{formatCurrency(deposit, currency)}</span>
                        </Typography>
                    )}
