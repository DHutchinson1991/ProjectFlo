...
import { ActivitiesCard } from '../package-template/ActivitiesCard';
...
import {
    ...
    type InstanceOwner,
} from '../../hooks/instance/useInstanceScheduleData';
import {
    ScheduleApiProvider,
    useScheduleApi,
    ...
    type ScheduleApi,
} from '../shared/ScheduleApiContext';
...
function ScheduleEditorContent({
    ...
    const { eventDays, activities, crewSlots, subjects, locationSlots, films } = data;
    const { crew, jobRoles, subjectTemplates, allEquipment } = refData;
    ...
    // but also has a relational fallback (from crew slot→equipment links).
    // We provide an empty shell so the component mounts, and actual equipment
    // is populated from the relational crew slot data.
    ...
    return (
        <Box>
            ...
            {/* ── Main grid: Activities | Subjects+Locations | Crew+Equipment | Task Auto-Gen ── */}
            <ScheduleCardGrid
                col1={<>
                    <ActivitiesCard
                        packageId={null}
                        ...
                        PackageCrewSlots={crewSlots}
                        setPackageCrewSlots={setCrewSlots}
                        selectedActivityId={selectedActivityId}
                        onSelectedActivityChange={setSelectedActivityId}
                    ...
                    <DeliverablesCard
                        ...
                        onFilmClick={(pf) => {
                            const linkedActivityIds = Array.from(
                                new Set(
                                    (pf.scene_schedules || [])
                                        .map((s: any) => s.project_activity_id)
