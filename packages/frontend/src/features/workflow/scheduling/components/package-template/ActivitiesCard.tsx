        ...
        if (savedActivityId && setPackageCrewSlots) {
            for (const change of crewChanges) {
                try {
                    let updatedOp;
                    if (change.assign) {
                        updatedOp = await crewSlotApi.assignActivity(change.id, savedActivityId);
                    } else {
                        updatedOp = await crewSlotApi.unassignActivity(change.id, savedActivityId);
                    }
                            ...
                            <Table size="small" sx={{ tableLayout: 'fixed', width: '100%' }}>
                                ...
                                <TableBody>
                                    {dayActivities.map((act) => {
                                        ...
                                        // Mirrors CrewCard.isCrewAssigned: slots with no explicit assignments
                                        // are "global" (available to all activities) — not just legacy single-activity ones.
                                        const _isAssignedToCrew = (o: any) => // eslint-disable-line @typescript-eslint/no-explicit-any
                                            ...
                                            (!o.activity_assignments?.length && !o.package_activity_id) ||
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <TableCell sx={{ py: 1, px: 1.5 }}>
                                                    <Typography sx={{ fontSize: '0.8rem', color: '#94a3b8', fontFamily: 'monospace', whiteSpace: 'nowrap' }}>
                                                ...
                                                {/* Duration */}
                                                <TableCell sx={{ py: 1, px: 1.5 }}>
                                                    <Typography sx={{ fontSize: '0.8rem', color: '#64748b', fontFamily: 'monospace', whiteSpace: 'nowrap' }}>
                                                ...
                                                <TableCell sx={{ py: 1, px: 1.5 }}>
                                                    <Box
                                                        ...
                                                        sx={{ display: 'flex', alignItems: 'center', gap: 1, justifyContent: 'center', opacity: 0.3, transition: 'opacity 0.15s' }}
                                                    >
                                                        <Tooltip title="Edit">
                                                            <IconButton
                                                                size="small"
