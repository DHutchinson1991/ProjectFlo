                            ...
                            onClick={async (e) => {
                                ...
                                try {
                                    if (contextApi?.brandEventDays) {
                                        const all = await contextApi.brandEventDays.getAll(brandId) as EventDay[];
                                        setBrandEventDays(all);
                                    } else {
                                        const all = await workflowScheduleApi.eventDays.getAll(brandId) as EventDay[];
                                        setBrandEventDays(all);
                                    }
                                } catch { /* use cache */ }
                                    ...
                                    .map(bd => (
                                        <Box
                                            ...
                                            onClick={async () => {
                                                try {
                                                    if (packageId) {
                                                        const res = await workflowScheduleApi.packageEventDays.add(packageId, bd.id) as EventDayJoinResult;
                                                        ...
                                                        const res = await contextApi.eventDays.create({ name: bd.name, event_day_template_id: bd.id }) as EventDayCreateResult;
                            ...
                            <TextField
                                ...
                                onKeyDown={async (e) => {
                                    if (e.key === 'Enter' && newDayName.trim() && (packageId || contextApi)) {
                                        try {
                                            if (packageId) {
                                                const created = await workflowScheduleApi.eventDays.create(brandId, { name: newDayName.trim() }) as EventDayCreateResult;
                                                const res = await workflowScheduleApi.packageEventDays.add(packageId, created.id) as EventDayJoinResult;
                                                setPendingDayId(created.id);
                                                setPendingJoinId(res._joinId ?? res.id);
                                                setPendingDayName(created.name || newDayName.trim());
                                                setPackageEventDays(prev => [...prev, {
                                                    id: created.id,
                                                    name: created.name || newDayName.trim(),
                                                    order_index: created.order_index ?? prev.length,
                                                    _joinId: res._joinId ?? res.id,
                                                }]);
                                            } else if (contextApi) {
                                                const res = await contextApi.eventDays.create({ name: newDayName.trim() }) as EventDayCreateResult;
                                                setPendingDayId(res.id);
                                                setPendingJoinId(res.id);
