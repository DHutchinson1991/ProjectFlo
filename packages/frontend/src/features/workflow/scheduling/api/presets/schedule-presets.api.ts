...
export function createSchedulePresetsApi(client: ApiClient) {
    return {
        presets: {
            getAll: (brandId: number): Promise<unknown[]> =>
                client.get(`/api/schedule/presets/brand/${brandId}`),
            upsert: (brandId: number, data: { name: string; schedule_data: unknown[] }): Promise<unknown> =>
                client.post(`/api/schedule/presets/brand/${brandId}`, data),
            rename: (brandId: number, presetId: number, name: string): Promise<unknown> =>
        ...
        eventDays: {
            ...
            create: (brandId: number, data: { name: string; description?: string; order_index?: number }): Promise<unknown> =>
                client.post(`/api/schedule/event-days/brand/${brandId}`, data),
            update: (brandId: number, id: number, data: unknown): Promise<unknown> =>
        ...
        activityPresets: {
            getAll: (eventDayId: number): Promise<unknown[]> =>
                client.get(`/api/schedule/event-days/${eventDayId}/activity-presets`),
            create: (eventDayId: number, data: { name: string; color?: string; icon?: string; default_duration_minutes?: number; order_index?: number }): Promise<unknown> =>
                client.post(`/api/schedule/event-days/${eventDayId}/activity-presets`, data),
            bulkCreate: (eventDayId: number, presets: { name: string; color?: string; order_index?: number }[]): Promise<unknown> =>
                client.post(`/api/schedule/event-days/${eventDayId}/activity-presets/bulk`, { presets }),
            update: (presetId: number, data: unknown): Promise<unknown> =>
        ...
        presetMoments: {
            getAll: (presetId: number): Promise<unknown[]> =>
                client.get(`/api/schedule/activity-presets/${presetId}/moments`),
            create: (presetId: number, data: { name: string; duration_seconds?: number; order_index?: number; is_key_moment?: boolean }): Promise<unknown> =>
                client.post(`/api/schedule/activity-presets/${presetId}/moments`, data),
            bulkCreate: (presetId: number, moments: { name: string; duration_seconds?: number; order_index?: number; is_key_moment?: boolean }[]): Promise<unknown> =>
        ...
        film: {
            get: (filmId: number): Promise<unknown> =>
                client.get(`/api/schedule/films/${filmId}`),
            upsertScene: (filmId: number, data: unknown): Promise<unknown> =>
                client.post(`/api/schedule/films/${filmId}/scenes`, data),
            bulkUpsertScenes: (filmId: number, schedules: unknown[]): Promise<unknown[]> =>
                client.post(`/api/schedule/films/${filmId}/scenes/bulk`, schedules),
            updateScene: (scheduleId: number, data: unknown): Promise<unknown> =>
