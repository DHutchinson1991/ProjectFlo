/**
 * Subjects Tab - read-only from package subjects when `packageId` is provided,
 * or editable `SubjectsCard` when running in film-only mode.
 */
import React, { useState, useEffect } from "react";
import { Box, Typography, Chip, CircularProgress, Alert } from "@mui/material";
import PeopleIcon from "@mui/icons-material/People";
import { type Subject, type SubjectTemplate, rolesApi, SubjectsCard } from '@/features/content/subjects';
...
export const SubjectsTab: React.FC<SubjectsTabProps> = ({
    ...
    const [typeTemplates, setTypeTemplates] = useState<RoleTemplate[]>([]);
    ...
    useEffect(() => {
        ...
        if (brandId) {
            fetches.push(
                rolesApi.getRoles(brandId)
                    .catch(() => [])
            );
        }
        Promise.all(fetches)
            .then(([subs, days, templates]) => {
                setPackageSubjects((subs || []) as PackageSubject[]);
                setEventDays((days || []) as EventDay[]);
                setTypeTemplates((templates || []) as RoleTemplate[]);
            })
    ...
    if (packageId) {
        ...
        type DayEntry = { day: EventDay; roles: string[] };
        const dayEntries: DayEntry[] = eventDays.map(day => {
            ...
            const templateRoles: string[] = matched.flatMap((t) =>
                (t.roles || []).map((r) => r.role_name as string)
            );
            const explicitNames: string[] = packageSubjects
                .filter((s) => s.event_day_template_id === day.id)
        ...
        const knownDayIds = new Set(eventDays.map((d) => d.id));
        const ungrouped = packageSubjects.filter((s) => s.event_day_template_id != null && !knownDayIds.has(s.event_day_template_id));
