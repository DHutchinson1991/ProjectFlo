...
type TemplateMomentLike = {
    id: number;
    name: string;
    duration?: number;
    order_index?: number | null;
    recording_setup?: MomentRecordingSetupLike | null;
};
...
export const applyEquipmentTemplateToFilm = async (
    ...
    try {
        const templates = await equipmentApi.getTemplatesByBrand(brandId);
        ...
        const existingAssignments = await filmsApi.equipmentAssignments.getAll(filmId) as EquipmentAssignmentLike[];
        ...
        for (const item of eligibleItems) {
            ...
            try {
                await filmsApi.equipmentAssignments.assign(filmId, {
...
export const createLinkedFilmFromTemplate = async ({
    ...
    const applySubjectTemplateToFilm = async (filmId: number, templateId?: number | null) => {
        if (!templateId) return;
        try {
            const templates = await rolesApi.getRoleTemplates(brandId) as SubjectRoleTemplateLike[];
            const template = templates.find((t) => t.id === templateId);
            if (!template || !Array.isArray(template.roles)) return;

            for (const role of template.roles) {
                try {
                    await subjectsApi.createSubject(filmId, {
                        name: role.role_name,
    ...
    const templateFilm = await filmsApi.films.getById(templateFilmId);
    ...
    const newTracks = (await filmsApi.tracks.getAll(newFilm.id)) as TrackLike[];
    const trackIdMap = buildTrackIdMap(templateTracks, newTracks);

    const templateScenes = (await scenesApi.scenes.getByFilm(templateFilmId)) as SceneLike[];
    const sortedScenes = [...(templateScenes || [])].sort(
        (a, b) => (a.order_index ?? 0) - (b.order_index ?? 0)
    );

    for (const scene of sortedScenes) {
        const createdScene = await filmsApi.localScenes.create(newFilm.id, {
            name: scene.name,
        ...
        const templateMoments = await scenesApi.moments.getByScene(scene.id) as TemplateMomentLike[];
        for (const moment of templateMoments || []) {
            ...
            if (moment.recording_setup) {
                const rs = moment.recording_setup;
        ...
        const templateBeats = scene.beats?.length
            ? scene.beats
            : await beatsApi.getSceneBeats(scene.id);

        for (const beat of templateBeats || []) {
            const createdBeat = await beatsApi.create(createdScene.id, {
                name: beat.name,
            ...
            if (beat.recording_setup) {
                await beatsApi.recordingSetup.upsert(createdBeat.id, {
