...
import { quotesApi } from '@/features/finance/quotes';
...
import type { Quote, QuoteItem, QuotePaymentMilestone } from '@/features/finance/quotes/types';
...
const QuotesCard: React.FC<WorkflowCardProps> = ({ inquiry, onRefresh, isActive, activeColor }) => {
    ...
    useEffect(() => {
        ...
        if (prefId) {
            paymentSchedulesApi.getById(prefId)
    ...
    const loadMilestones = async (quoteId: number) => {
        try {
            const ms = await paymentSchedulesApi.getQuoteMilestones(quoteId);
            setMilestones(ms || []);
        } catch {
            setMilestones([]);
        }
    ...
    useEffect(() => {
        const fetchQuotes = async () => {
            if (inquiry?.id) {
                try {
                    const quotesData = await quotesApi.getAllByInquiry(inquiry.id);
    ...
    const fetchEstimates = async () => {
        ...
        try {
            const estimates = await estimatesApi.getAllByInquiry(inquiry.id);
    ...
    const handleSave = async (statusOverride?: string) => {
        try {
            ...
            if (editingQuote && editingQuote.id) {
                await quotesApi.update(inquiry.id, editingQuote.id, quoteData);
                savedId = editingQuote.id;
            } else {
                const created = await quotesApi.create(inquiry.id, quoteData);
                savedId = created?.id;
            }
            ...
            try {
                ...
                const updatedQuotes = await quotesApi.getAllByInquiry(inquiry.id);
    ...
    const handleDelete = async (quoteId: number) => {
        ...
        try {
            await quotesApi.delete(inquiry.id, quoteId);
            setDialogOpen(false);
    ...
    const handleSetFocus = async (quoteId: number, e?: React.MouseEvent) => {
        ...
        try {
            await quotesApi.update(inquiry.id, quoteId, { is_primary: true });
