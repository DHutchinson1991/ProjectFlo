...
export default function DeliverableDetailPage() {
  ...
  const [visualTemplate, setVisualTemplate] = useState<VisualTemplate | null>(
    null,
  );
  ...
  const convertToVisualTemplate = (
    deliverable: DeliverableTemplate,
  ): VisualTemplate => {
    console.log("Converting deliverable:", deliverable.name);
    console.log(
      "Has default components:",
      deliverable.template_defaults?.length || 0,
    );
    console.log(
      "Has regular components:",
      deliverable.assigned_components?.length || 0,
    );
    ...
    if (regularComponents.length > 0) {
      ...
      regularComponents.forEach((comp, index) => {
        ...
        convertedComponents.push({
          ...
          duration:
            comp.duration_override || comp.component?.estimated_duration || 10,
          component: {
            ...
            category:
              comp.editing_style ||
              (comp.component?.type === "COVERAGE_BASED"
                ? "Main Content"
                : "Production"),
      ...
      defaultComponents.forEach((comp, index) => {
        const compName = comp.component?.name || `Component ${index + 1}`;
        const compDesc = comp.component?.description || "Component";

        convertedComponents.push({
          id: `dc-${comp.id}`,
          componentId: comp.component_id.toString(),
          order: comp.order_index || index,
          duration:
            comp.duration_override || comp.component?.estimated_duration || 10,
          component: {
            id: comp.component_id.toString(),
            ...
            baseTimeMinutes: comp.component?.estimated_duration || 10,
            complexityMultiplier: comp.component?.complexity_score || 1.0,
            baseCost: parseFloat(comp.component?.base_task_hours || "50"),
            icon: "Camera",
            color: "#2196F3",
          },
        });
    ...
    const totalDuration = convertedComponents.reduce(
      (sum, comp) => sum + (comp.duration || 0),
      0,
    );
    const totalHours = regularComponents.reduce(
      (sum: number, comp) =>
        sum + parseFloat(comp.calculated_task_hours || "0"),
      0,
    );
    const estimatedCost = regularComponents.reduce(
      (sum: number, comp) =>
        sum + parseFloat(comp.calculated_base_price || "0"),
      0,
    );
    const complexity =
      regularComponents.length > 0
        ? regularComponents.reduce(
            (sum: number, comp) =>
              sum + (comp.component?.complexity_score || 1),
            0,
  ...
  const saveTemplateChanges = async (template: VisualTemplate) => {
    try {
      ...
      // Save to backend
      const updatedDeliverable = await deliverableAPI.updateTemplate(
        deliverable.id,
        updateData,
      );
      ...
      if (components.length > 0) {
        const updatedWithComponents = await deliverableAPI.updateComponents(
          deliverable.id,
          components,
        );
        console.log(
          "Components saved successfully, updated deliverable:",
          updatedWithComponents.name,
        );
        ...
        const refreshedTemplate = convertToVisualTemplate(
          updatedWithComponents,
        );
          ...
          <Box sx={{ mb: 3 }}>
            ...
            <Collapse in={componentsExpanded}>
              <DeliverableComponentBuilder
                ...
                availableComponents={convertComponentsForBuilder(
                  availableComponents,
                )}
              />
            </Collapse>
        ...
        <Grid item xs={12}>
          {deliverable && (
            <DeliverableTaskManager
              ...
              componentIds={
                deliverable.assigned_components?.map(
