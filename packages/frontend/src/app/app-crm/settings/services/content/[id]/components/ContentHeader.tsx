...
import { categoryAPI, contentAPI } from '../../../_shared/api';
import { ContentCategory, ContentTemplate } from '../../../_shared/types';

interface ContentHeaderProps {
    content: ContentTemplate;
    onContentUpdated: (updatedContent: ContentTemplate) => void;
}
...
export default function ContentHeader({
    ...
    const [titleValue, setTitleValue] = useState(content.name || '');
    const [descriptionValue, setDescriptionValue] = useState(content.description || '');
    ...
    const handleCategoryChange = async (newCategoryCode: string) => {
        try {
            ...
            const updatedContent = await contentAPI.update(content.id, {
                type: newCategoryCode as ContentTemplate['type']
            });

            onContentUpdated(updatedContent);
            setIsEditing(false);
        } catch (err) {
            console.error('Failed to update content category:', err);
        } finally {
            setUpdating(false);
        }
    };
    ...
    const getCurrentCategory = () => {
        return categories.find(cat => cat.code === content.type);
    };
        ...
        <>
            <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', mb: 3 }}>
                <Box sx={{ display: 'flex', alignItems: 'flex-start', flex: 1 }}>
                    <Link href="/app-crm/settings/services/content" passHref>
