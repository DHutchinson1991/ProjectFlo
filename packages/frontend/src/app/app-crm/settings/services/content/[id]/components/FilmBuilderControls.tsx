...
import { PlaybackState, ViewState } from "./FilmBuilderTypes";

interface FilmBuilderControlsProps {
...
const FilmBuilderControls: React.FC<FilmBuilderControlsProps> = ({
    playbackState,
    viewState,
    onPlay,
    onStop,
            ...
            >
                ...
                <Stack direction="row" spacing={0.5} alignItems="center">
                    <Tooltip title={playbackState.isPlaying ? "Pause" : "Play"}>
                        ...
                        >
                            {playbackState.isPlaying ? <PauseIcon fontSize="small" /> : <PlayIcon fontSize="small" />}
                        </IconButton>
                    </Tooltip>
                    ...
                    >
                        ...
                        >
                            {playbackSpeeds.map((speed) => (
                                <MenuItem key={speed} value={speed} sx={{ fontSize: "0.75rem" }}>
                                    {speed}x
                                </MenuItem>
                            ))}
                        </Select>
                ...
                <Box sx={{ flex: 1, mx: 3, display: "flex", alignItems: "center", gap: 2 }}>
                    ...
                    <Box sx={{ flex: 1, py: 0.5 }}> {/* Reduced padding since no value label */}
                        <Slider
                            value={playbackState.currentTime}
                            min={0}
                            max={playbackState.totalDuration}
                            onChange={(_, value) => onTimelineClick(Array.isArray(value) ? value[0] : value)}
                            sx={{
                ...
                <Stack direction="row" spacing={0.5} alignItems="center">
                    ...
                    <Tooltip title="Zoom In">
                        <IconButton
                            onClick={() => onZoomChange(Math.min(50, viewState.zoomLevel + 5))}
                            sx={{
                                color: "#888",
                                width: 28,
                                height: 28,
...
export default FilmBuilderControls;
