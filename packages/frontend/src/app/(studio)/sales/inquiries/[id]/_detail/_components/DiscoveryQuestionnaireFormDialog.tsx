...
export default function DiscoveryQuestionnaireFormDialog({
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            api.inquiries.scheduleSnapshot.getActivities(inquiryId).catch(() => [] as SnapshotActivity[]),
            brandId > 0 ? api.brands.getMeetingSettings(brandId).catch(() => null) : Promise.resolve(null),
            brandId > 0
                ? api.inquiries.getById(inquiryId)
                    .then((inq) => inq.preferred_payment_schedule_template_id
                        ? api.paymentSchedules.getById(brandId, inq.preferred_payment_schedule_template_id)
                        : api.paymentSchedules.getDefault(brandId))
                    .catch(() => null)
