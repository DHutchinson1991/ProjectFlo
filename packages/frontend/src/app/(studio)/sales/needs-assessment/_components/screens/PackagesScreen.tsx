            ...
            {filteredPackages.length > 0 ? (
                <Box sx={{
                    ...
                    {filteredPackages.map((pkg) => {
                        ...
                        const backendTax = (pkg as any)._tax as { rate: number; amount: number; totalWithTax: number } | null | undefined;
                        const totalCost = backendTax?.totalWithTax ?? Number((pkg as any)._totalCost ?? 0);
                        ...
                        const filmItems = items.filter((it: any) => it.type === "film");
                        const serviceItems = items.filter((it: any) => it.type === "service");
                                ...
                                <Box sx={{ p: 3, display: "flex", flexDirection: "column", flex: 1 }}>
                                    ...
                                    {(hasTypedItems ? serviceItems.length > 0 : false) && (
                                        <Box sx={{ mb: 2 }}>
                                            ...
                                            <Stack spacing={0.55}>
                                                {serviceItems.slice(0, 4).map((item: any, i: number) => (
                                    ...
                                    {(hasTypedItems ? filmItems.length > 0 : false) && (
                                        <Box sx={{ mb: 2 }}>
                                            ...
                                            <Stack spacing={0.55}>
                                                {filmItems.slice(0, 4).map((item: any, i: number) => (
                                                    <Box key={i} sx={{ display: "flex", alignItems: "center", gap: 0.9 }}>
