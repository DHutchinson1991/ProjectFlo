...
import { locationsApi, FloorPlan, UpdateFloorPlanDto } from '@/lib/api/locations';
...
export default function FloorPlanEditorPage() {
    ...
    const floorPlanId = parseInt(params.floorPlanId as string);
    ...
    const loadFloorPlan = async () => {
        try {
            setLoading(true);
            const data = await locationsApi.getFloorPlan(floorPlanId);
            setFloorPlan(data);

            // Load existing objects if available
    ...
    const saveFloorPlan = async () => {
        ...
        try {
            ...
            const updateData: UpdateFloorPlanDto = {
    ...
    if (!floorPlan) {
        return <Box>Floor plan not found</Box>;
    }
    ...
    return (
        <Box sx={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <Box sx={{ display: 'flex', flex: 1 }}>
                ...
                <Card sx={{ width: 250, m: 1, display: 'flex', flexDirection: 'column' }}>
                    <CardContent sx={{ flex: 1 }}>
                        ...
                        <Box display="flex" gap={1} mb={2}>
                            <IconButton onClick={() => setZoom(zoom * 1.2)} title="Zoom In">
                                <ZoomInIcon />
                            </IconButton>
                            <IconButton onClick={() => setZoom(zoom * 0.8)} title="Zoom Out">
                                <ZoomOutIcon />
                            </IconButton>
                            <IconButton onClick={() => setZoom(1)} title="Center">
                                <CenterIcon />
                            </IconButton>
                        </Box>
                    ...
                    <Card sx={{ flex: 1, overflow: 'hidden' }}>
                        <Box sx={{ p: 2, borderBottom: 1, borderColor: 'divider' }}>
                            <Typography variant="h6">
                                Canvas (800×600) - {canvasObjects.length} objects
                            </Typography>
                        </Box>
                        <Box
                            sx={{
                                ...
                                {canvasObjects.map((obj) => (
                                    <Box
                                        ...
                                        sx={{
                                            ...
                                            border: `${obj.strokeWidth || 1}px solid ${obj.stroke || '#000'}`,
                                            display: 'flex',
                                            alignItems: 'center',
                                ...
                                {canvasObjects.length === 0 && (
                                    ...
                                    >
                                        ...
                                        <Typography variant="body2">
                                            Add objects from the library to start designing
                                        </Typography>
                                    </Box>
                                )}
                            </Box>
                ...
                <Card sx={{ width: 250, m: 1 }}>
                    <CardContent>
                        ...
                        <Typography variant="body2" gutterBottom>
                            <strong>Name:</strong> {floorPlan.name}
                        </Typography>
                        <Typography variant="body2" gutterBottom>
                            <strong>Version:</strong> {floorPlan.version}
                        </Typography>
                        <Typography variant="body2" gutterBottom>
                            <strong>Objects:</strong> {canvasObjects.length}
                        </Typography>
                        <Typography variant="body2" gutterBottom>
                            <strong>Canvas Size:</strong> 800×600
                        </Typography>

                        <Divider sx={{ my: 2 }} />

                        <Typography variant="body2" color="text.secondary">
                            <strong>Note:</strong> This is a simplified floor plan editor.
                            Full Fabric.js integration is planned for future updates with
                            advanced drawing tools, precise measurements, and more object types.
                        </Typography>
                    </CardContent>
                </Card>
            </Box>
                        ...
                        {OBJECT_LIBRARY.map((item) => (
                            <Grid item xs={6} sm={4} key={item.id}>
                                <Card
                                    ...
                                    onClick={() => addObjectFromLibrary(item)}
                                >
            ...
            {/* Floating Action Button for Object Library on mobile */}
            <Fab
                color="primary"
                sx={{
                    position: 'fixed',
