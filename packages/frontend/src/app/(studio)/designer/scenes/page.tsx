...
export default function ScenesLibraryPage() {
    ...
    // Query scenes with brand context
    const {
        data: scenes = [],
        isLoading: loading,
        error: queryError,
    ...
    const createMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["scenes", currentBrand?.id] });
    ...
    const updateMutation = useMutation({
        mutationFn: ({ id, data }: { id: number; data: any }) => api.scenes.update(id, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["scenes", currentBrand?.id] });
    ...
    React.useEffect(() => {
        if (queryError) {
            setError(queryError instanceof Error ? queryError.message : "Failed to load scenes");
        } else {
            setError(null);
        }
    }, [queryError]);
    ...
    // Show loading if brand is not selected
    if (!currentBrand) {
        return (
            <Box sx={{ p: 3 }}>
                <Alert severity="info">Please select a brand to view scenes.</Alert>
            </Box>
        );
    }
