...
const ContentBuilderTimeline: React.FC<ContentBuilderTimelineProps> = ({
  ...
  const getSnapGridSeparator = () => {
    ...
    const markerInterval =
      viewState.zoomLevel < 15 ? 30 : viewState.zoomLevel < 25 ? 15 : 5;

    // Simple snap markers - just tiny dots, perfectly centered
    for (
      let time = 0;
      time <= playbackState.totalDuration;
      time += viewState.gridSize
    ) {
      gridMarkers.push(
        <Box
          key={`snap-${time}`}
  ...
  return (
    <Box sx={{ width: "100%", height: "100%" }}>
        ...
        >
          ...
          >
            ...
            {tracks.map((track, index) => {
              ...
              const isAudioTrack =
                track.track_type === "audio" || track.track_type === "music";
                  ...
                  {isValidDropTarget &&
                    (dragState.draggedLibraryScene ||
                      ...
                      >
                        ...
                        >
                          {dragState.draggedLibraryScene
                            ? `Drop ${dragState.draggedLibraryScene.name} here`
                            : dragState.draggedScene
                              ? `Move ${dragState.draggedScene.name} here`
                              : "Drop component here"}
                        </Typography>
                      </Box>
                    )}
            ...
            {scenes.map((scene) => {
              ...
              // Calculate position with separator gap
              let sceneTop = trackIndex * 40 + 8;
              if (isAudioTrack) {
                // Add clean separator gap for audio tracks
                sceneTop += 24;
              }

              const isSelected = viewState.selectedScene?.id === scene.id;
              const isDragging = dragState.draggedScene?.id === scene.id;
              ...
              return (
                <Box
                  ...
                  sx={{
                    position: "absolute",
                    left: scene.start_time * viewState.zoomLevel,
                    top: sceneTop,
                    width: Math.max(scene.duration * viewState.zoomLevel, 20),
                    height: 24,
                    bgcolor: scene.color,
                    borderRadius: 1,
                    border: isDragging
                    ...
                    boxShadow: isDragging
                      ...
                      : isSelected
                        ...
                        : "none",
                    transition:
                      "border-color 0.1s ease-in-out, box-shadow 0.1s ease-in-out, opacity 0.1s ease-in-out",
                    display: "flex",
                    alignItems: "center",
                    px: 0.75,
                    overflow: "hidden",
                    ...
                    "&:active": readOnly
                      ? {}
                      : {
                          cursor: scene.locked ? "not-allowed" : "grabbing",
                        },
                  }}
                  onMouseDown={
                    !readOnly && !scene.locked
                      ? (e) => onSceneMouseDown(e, scene)
                      : undefined
                  }
                >
                  {/* Scene content */}
      ...
      >
        ...
        >
          {scenes.length} scenes • {formatTime(playbackState.totalDuration)} •{" "}
