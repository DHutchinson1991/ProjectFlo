...
export const useMusic = () => {
  ...
  const loadSceneMusic = useCallback(async (sceneId: number) => {
    ...
    setError(null);
    try {
      const data = await request<SceneMusic>(`/music/scenes/${sceneId}/music`);
      setSceneMusic(data);
      return data;
    } catch (err) {
  ...
  const loadMomentMusic = useCallback(async (momentId: number) => {
    ...
    setError(null);
    try {
      const data = await request<MomentMusic>(`/music/moments/${momentId}/music`);
      setMomentMusic(data);
      return data;
    } catch (err) {
  ...
  const createSceneMusic = useCallback(async (sceneId: number, payload: CreateSceneMusicDto) => {
    const created = await request<SceneMusic>(`/music/scenes/${sceneId}/music`, {
      method: "POST",
  ...
  const updateSceneMusic = useCallback(async (sceneId: number, payload: UpdateSceneMusicDto) => {
    const updated = await request<SceneMusic>(`/music/scenes/${sceneId}/music`, {
      method: "PATCH",
      body: JSON.stringify(payload),
    });
  ...
  const removeSceneMusic = useCallback(async (sceneId: number) => {
    await request<void>(`/music/scenes/${sceneId}/music`, { method: "DELETE" });
    setSceneMusic(null);
  }, []);

  const createMomentMusic = useCallback(async (momentId: number, payload: CreateMomentMusicDto) => {
    const created = await request<MomentMusic>(`/music/moments/${momentId}/music`, {
      method: "POST",
      body: JSON.stringify(payload),
    });
  ...
  const updateMomentMusic = useCallback(async (momentId: number, payload: UpdateMomentMusicDto) => {
    const updated = await request<MomentMusic>(`/music/moments/${momentId}/music`, {
      method: "PATCH",
      body: JSON.stringify(payload),
    });
  ...
  const removeMomentMusic = useCallback(async (momentId: number) => {
    await request<void>(`/music/moments/${momentId}/music`, { method: "DELETE" });
    setMomentMusic(null);
  }, []);
