...
export async function createMoonriseTeam(db: PrismaClient, brandId: number) {
    ...
    if (soundEngineerRole) {
        ...
        await prisma.contributor_job_roles.upsert({
            where: { contributor_id_job_role_id: { contributor_id: andyContributor.id, job_role_id: soundEngineerRole.id } },
            update: { is_primary: false, payment_bracket_id: bracketId },
            create: { contributor_id: andyContributor.id, job_role_id: soundEngineerRole.id, is_primary: false, payment_bracket_id: bracketId },
        });
