...
/**
 * Activity Presets per Event Day Template.
 *
 * Seeds default activity suggestions for each event day type so the
 * "Add Activity" dialog offers relevant presets out-of-the-box.
 * Each preset also includes default moments with durations.
 */
...
async function seedActivityPresets(): Promise<SeedSummary> {
    logger.sectionHeader('Activity Presets', 'Schedule: Default activity presets per event day');
    logger.startTimer('activity-presets');
    ...
    if (!brand) {
        logger.warning('Moonrise Films brand not found, skipping activity presets.');
    ...
    if (templates.length === 0) {
        logger.warning('No event day templates found. Run event-day-templates seed first.');
        return { created: 0, updated: 0, skipped: 0, total: 0 };
    }
    ...
    for (const tpl of templates) {
        ...
        if (!presets) {
            logger.skipped(`"${tpl.name}"`, 'no preset definitions for this day type', 'verbose');
            skipped += 1;
            continue;
        }

        logger.processing(`Seeding presets for "${tpl.name}" (${presets.length} presets)...`);
        ...
        for (let i = 0; i < presets.length; i++) {
            ...
            if (existing) {
                ...
                created += 1;
                logger.created(`  "${p.name}" → ${tpl.name}`, undefined, 'verbose');
            }
    ...
    logger.endTimer('activity-presets', 'Activity presets seeding');
    return { created, updated, skipped, total };
}

export default seedActivityPresets;

// Allow running standalone
if (require.main === module) {
    seedActivityPresets()
        .then((summary) => {
            console.log('Activity presets seeding completed:', summary);
        })
        .catch((e) => {
            console.error('❌ Activity presets seeding failed:', e);
            process.exit(1);
        })
        .finally(async () => {
