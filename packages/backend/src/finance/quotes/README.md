...
| File | Purpose |
|------|---------|
| `quotes.service.ts` | Create, update, delete, send quotes; total computed from items |
| `mappers/quote-response.mapper.ts` | Decimal→number conversion + tax calculation for API responses |
...
- **Backend**: `../workflow/tasks` — quote acceptance can update inquiry task status
