...
import { buildBracketMap, buildExecRoleCrewMap, buildRateMaps, lookupRate, detectFilmsWithContent, PreviewRoleCrewMap, PreviewCrewMember } from '../utils/task-library-gen.functions';
...
interface OperatorRow {
    ...
    label: string | null;
    crew_member: { contact: { first_name: string | null; last_name: string | null } | null } | null;
...
export class TaskLibraryPreviewService {
    ...
    private async _core(packageId: number, brandId: number, inquiryId?: number, projectId?: number) {
        ...
        const [filmCount, eventDayCount, crewCount, locationCount, activityCount, activityCrewCount, filmSceneCount] = await Promise.all([
        ...
        const opInclude = { crew_member: { include: { contact: { select: { first_name: true, last_name: true } } } }, job_role: { select: { id: true, name: true, display_name: true } } } as const;
        ...
        if (instWhere) {
            ...
            operators = inst.length > 0 ? inst : await this.prisma.packageCrewSlot.findMany({ where: { package_id: packageId }, include: opInclude, orderBy: { order_index: 'asc' } });
        } else {
            operators = await this.prisma.packageCrewSlot.findMany({ where: { package_id: packageId }, include: opInclude, orderBy: { order_index: 'asc' } });
        }
        ...
        const execMap = buildExecRoleCrewMap(operators, bracketMap, validAssignments);
        const previewCrewMap: PreviewRoleCrewMap = new Map();
        for (const op of operators) {
            if (!op.job_role_id || !op.crew_member || !validAssignments.has(`${op.crew_member_id}-${op.job_role_id}`)) continue;
            const name = `${op.crew_member.contact?.first_name || ''} ${op.crew_member.contact?.last_name || ''}`.trim(); if (!name) continue;
            const bl = bracketMap.get(`${op.crew_member_id}-${op.job_role_id}`) ?? 0;
        ...
        const brackets = allRoleIds.size > 0 ? await this.prisma.payment_brackets.findMany({ where: { job_role_id: { in: [...allRoleIds] }, is_active: true }, select: { job_role_id: true, level: true, hourly_rate: true }, orderBy: [{ job_role_id: 'asc' }, { level: 'asc' }] }) : [];
        const { bracketRateMap, roleFallbackRate } = buildRateMaps(brackets);
        const jobRoleNameMap = new Map<number, string>();
        for (const op of operators) { if (op.job_role && !jobRoleNameMap.has(op.job_role.id)) jobRoleNameMap.set(op.job_role.id, op.job_role.display_name || op.job_role.name); }
        ...
        const [filmSceneDetails, activityCrewDetails, filmDetails, activityDetails, eventDayDetails] = await Promise.all([
            ...
            tasks.some(t => t.trigger_type === 'per_activity_crew') ? this.prisma.packageCrewSlotActivity.findMany({ where: { package_activity: { package_id: packageId } }, include: { package_activity: { select: { name: true, duration_minutes: true } }, package_crew_slot: { select: { label: true, crew_member_id: true, job_role_id: true, crew_member: { include: { contact: { select: { first_name: true, last_name: true } } } }, job_role: { select: { name: true, display_name: true } } } } }, orderBy: [{ package_activity: { order_index: 'asc' } }, { package_crew_slot: { order_index: 'asc' } }] }) : Promise.resolve([]),
        ...
        const activityCrewMapped: ActivityAssignment[] = activityCrewDetails.map(d => ({
            ...
            contributorId: d.package_crew_slot.crew_member_id,
            activityName: d.package_activity.name,
            durationMinutes: d.package_activity.duration_minutes,
            crewName: d.package_crew_slot.crew_member
                ? `${d.package_crew_slot.crew_member.contact?.first_name || ''} ${d.package_crew_slot.crew_member.contact?.last_name || ''}`.trim()
        ...
        const pickPreviewCrew = (roleId: number, taskBracketLevel: number | null): { name: string | null; bracketLevel: number | null } => {
            ...
            let best: PreviewCrewMember = list[0], bestDist = Math.abs(list[0].bracketLevel - taskBracketLevel);
        ...
        const triggerCounts: Record<string, number> = { always: 1, per_film: filmCount, per_film_with_music: filmsWithMusicPrev.length, per_film_with_graphics: filmsWithGraphicsPrev.length, per_event_day: eventDayCount, per_crew_member: crewCount, per_location: locationCount, per_activity: activityCount, per_activity_crew: activityCrewCount, per_film_scene: filmSceneCount };
        ...
        const generatedTasks = tasks.flatMap(task => {
            ...
            if (task.trigger_type === 'per_crew_member') return operators.filter(op => validAssignments.has(`${op.crew_member_id}-${op.job_role_id}`) || !op.crew_member_id).map(op => { const n = op.crew_member ? `${op.crew_member.contact?.first_name || ''} ${op.crew_member.contact?.last_name || ''}`.trim() : (op.label ?? op.job_role?.display_name ?? op.job_role?.name ?? 'Unknown'); return { task_library_id: task.id, name: `${task.name} — ${n}`, phase: task.phase, trigger_type: task.trigger_type, effort_hours_each: task.effort_hours ? Number(task.effort_hours) : 0, multiplier: 1, total_instances: 1, total_hours: task.effort_hours ? Number(task.effort_hours) : 0, role_name: op.job_role?.display_name || op.job_role?.name || a.role_name, assigned_to_name: n as string | null, hourly_rate: a.hourly_rate }; });
        ...
        const tasksWithCost: PreviewTaskRow[] = generatedTasks.filter(t => t.total_instances > 0).map(t => ({ ...t, estimated_cost: (t.hourly_rate !== null && t.hourly_rate !== undefined && t.total_hours > 0) ? roundMoney(t.hourly_rate * t.total_hours) : null, due_date_offset_days: offsetDaysMap.get(t.task_library_id) ?? null }));
        ...
        return { package: { id: pkg.id, name: pkg.name }, contentCounts: { films: filmCount, films_with_music: filmsWithMusicPrev.length, films_with_graphics: filmsWithGraphicsPrev.length, event_days: eventDayCount, crew_members: crewCount, locations: locationCount, activities: activityCount, activity_crew_assignments: activityCrewCount, film_scenes: filmSceneCount }, summary: { total_library_tasks: tasks.length, total_generated_tasks: tasksWithCost.reduce((s, t) => s + t.total_instances, 0), total_estimated_hours: roundMoney(tasksWithCost.reduce((s, t) => s + t.total_hours, 0)), total_estimated_cost: roundMoney(tasksWithCost.reduce((s, t) => s + (t.estimated_cost ?? 0), 0)) }, byPhase, tasks: tasksWithCost };
    }
    ...
    private _expandActivityCrew(task: { id: number; name: string; phase: string; trigger_type: string; effort_hours: unknown; default_job_role?: { id: number } | null }, assignments: ActivityAssignment[], a: { role_name: string | null; assigned_to_name: string | null; hourly_rate: number | null }, validAssignments: Set<string>, pickPreviewCrew: (roleId: number, bl: number | null) => { name: string | null; bracketLevel: number | null }, bracketRateMap: Map<string, number>, roleFallbackRate: Map<number, number>) {
        ...
        return relevant.filter((d) => !d.contributorId || !d.jobRoleId || validAssignments.has(`${d.contributorId}-${d.jobRoleId}`)).map((d) => {
