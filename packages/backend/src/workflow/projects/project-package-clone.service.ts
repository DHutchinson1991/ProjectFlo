 ...
 *   6. PackageCrewSlot         → ProjectCrewSlot
 *   7. PackageCrewSlotEquipment → ProjectCrewSlotEquipment
 *   8. PackageFilm                → ProjectFilm
 *   9. PackageFilmSceneSchedule   → ProjectFilmSceneSchedule
 *  10. PackageCrewSlotActivity  → ProjectCrewSlotActivity
...
export class ProjectPackageCloneService {
  ...
  async clonePackageToOwner(
    ...
    if (!target.projectId && !target.inquiryId) {
      throw new BadRequestException('CloneTarget must specify either projectId or inquiryId');
    }
    if (target.projectId && target.inquiryId) {
      throw new BadRequestException('CloneTarget cannot specify both projectId and inquiryId');
    }
  ...
  private async _clone(
    ...
    const contentsItems: Array<{ type: string; config?: { package_film_id?: number } }> =
      (pkg.contents as Prisma.JsonObject | null)?.items as typeof contentsItems ?? [];
    ...
    // ── 10. Clone PackageCrewSlotActivity → ProjectCrewSlotActivity
    const opAssignments = await prisma.packageCrewSlotActivity.findMany({
      where: {
        package_crew_slot: { package_id: packageId },
      },
    });
  ...
  private async _deleteInstanceData(
    ...
    const where = owner as Record<string, number>;
      ...
      if (locationName) {
        ...
        if (brandId !== null) {
          ...
          if (!libEntry) {
            ...
            const coords = await this.geocodingService.geocodeAddress(locationName);
