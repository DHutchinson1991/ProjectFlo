...
export class ProjectFilmCloneService {
  ...
  async cloneFilmContent(
    ...
    if (!target.projectId && !target.inquiryId) {
      throw new BadRequestException('FilmCloneTarget must specify either projectId or inquiryId');
    }
    if (target.projectId && target.inquiryId) {
      throw new BadRequestException('FilmCloneTarget cannot specify both projectId and inquiryId');
    }
