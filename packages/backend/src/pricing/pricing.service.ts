...
export class PricingService {
   ...
   */
  async calculateComponentPricing(params: PricingCalculationParams): Promise<Record<number, ComponentPricingResult>> {
    const { component_ids, wedding_date, delivery_deadline, component_count } = params;
    ...
    // Calculate base pricing from task hours
    ...
    for (const componentId of component_ids) {
      if (basePricing[componentId]) {
        ...
        basePricing[componentId].final_price = Math.round(finalPrice * 100) / 100; // Round to 2 decimals
  ...
  async previewPricing(params: PricingCalculationParams) {
    ...
    const summary = {
      ...
      total_base_price: Object.values(pricing).reduce((sum, p) => sum + p.base_price, 0),
      total_final_price: Object.values(pricing).reduce((sum, p) => sum + p.final_price, 0),
      total_hours: Object.values(pricing).reduce((sum, p) => sum + p.total_hours, 0),
